stringsAnswers = {
   ...
   *
   * Example: reduceString('aaaabbbb', 1) should reduce to 'ab'
   * Example: reduceString('aaaabbbb', 2) should reduce to 'aabb'
   *
  ...
  reduceString: function reduceString(str, amount) {
    ...
    for(var i = 0; i < str.length; i++){
      if(str.charAt(i) !== str.charAt(i+1)){
        rStr.push(str.charAt(i).repeat(amount));
      }
    }
    return rStr.join('');
  },
   ...
   *
   * Example: reverseString('abc') should be 'bca' cba??
   *
